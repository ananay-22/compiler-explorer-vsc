...
3. **Verify Build Process Locally:**
   - Use the `run_command` tool to execute `npm run build` or `npm run compile`.
   - Use the `run_command` tool to execute `npm run test`.
   - Use the `run_command` tool to execute `npx vsce package --no-dependencies`.
   - Only proceed with the `git commit` and `git push` if these terminal commands succeed without throwing Webpack errors, test failures, or packaging constraint errors.
